...
- **U2 Atomicidad (FASE 26 U2)**: `commit()`, `commitWithActual()`, y `release()` ahora usan `DB::transaction()` + `lockReservation()` (SELECT FOR UPDATE en PG, fresh() en SQLite). Precondition checks (status, expiry) se ejecutan dentro del row lock scope para evitar TOCTOU races. Crash entre reservation commit y ledger write es imposible. Exactly-once: segundo commit/release lanza `InvalidArgumentException`.
- **U3 Job Hardening (FASE 26 U3)**: `ProcessWhatsAppStatusUpdate.failed()` marca `WebhookEvent` como `failed` con `error_code = 'job_exhausted'` cuando los reintentos se agotan. Idempotente (no-op si evento ya processed/failed). `ProcessIncomingWhatsAppMessage` tiene `$timeout = 60` explícito (safe vs `retry_after=90`). Ambos jobs tienen timeout, tries y backoff declarados explícitamente.
- **U4 LIKE Escaping (FASE 26 U4)**: `FlowWebhookController::resolveByPhone()` ahora escapa `%`, `_`, `\` en el input del usuario con `escapeLike()` y usa `LIKE ? ESCAPE '\'` para prevenir bypass de búsqueda por expansión de wildcards.
- **U4 Provider Error Sanitization (FASE 26 U4)**: Los errores raw de proveedores externos (Meta, OpenAI, Stripe) NUNCA se exponen en mensajes de excepción. Se registran en `Log::warning()` con contexto estructurado (`whatsapp.meta_api_error`, `whatsapp.meta_connection_error`, `ai.openai_api_error`, `billing.stripe_api_error`). Los mensajes de excepción son genéricos y seguros. Global exception renderers en `bootstrap/app.php` para `WhatsAppException`, `AIException`, `BillingProviderException` actúan como safety net.
