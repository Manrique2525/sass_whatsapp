...
<template>
    <PlatformLayout :user="page.props.auth.user">
        ...
        <section class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="app-card p-5"><p class="app-eyebrow">Plan</p><p class="mt-2 font-semibold">{{ props.customer.plan?.name ?? 'No subscription' }}</p><p class="mt-1 text-sm text-[#71877b]">{{ label(props.customer.subscription?.status ?? null) }}</p></div>
        ...
        <div class="mt-6 grid gap-6 lg:grid-cols-2">
            <section class="app-card p-6"><h2 class="text-lg font-semibold">Subscription</h2><dl class="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div><dt class="text-[#71877b]">Status</dt><dd class="mt-1 font-medium">{{ label(props.customer.subscription?.status ?? null) }}</dd></div><div><dt class="text-[#71877b]">Provider</dt><dd class="mt-1 font-medium">{{ props.customer.subscription?.provider ?? 'Not connected' }}</dd></div><div><dt class="text-[#71877b]">Current period</dt><dd class="mt-1 font-medium">{{ formatDate(props.customer.subscription?.current_period_start ?? null) }} to {{ formatDate(props.customer.subscription?.current_period_end ?? null) }}</dd></div><div><dt class="text-[#71877b]">Cancellation</dt><dd class="mt-1 font-medium">{{ props.customer.subscription?.cancel_at_period_end ? 'At period end' : 'Not scheduled' }}</dd></div></dl><p v-if="props.customer.subscription?.provider_subscription_id" class="mt-4 text-xs text-[#71877b]">Provider reference: {{ props.customer.subscription.provider_subscription_id }}</p></section>
